...
class DB_Search_Widget extends DB_WP_Widget {
	...
	function outputStylesheets(){
		$dir = WP_CONTENT_URL .'/themes/marketplace/plugins/' .  dirname(plugin_basename(__FILE__) ) . '/';
...
class CustomSearchField extends SearchFieldBase {
	...
	function getInput($wrap=true){
		...
		if($wrap){
			$input = "<div class='searchform-param'><div class='label-wrap'><label class='searchform-label'>".$this->getLabel()."</label></div><div class='field-wrap'><span class='searchform-input-wrapper'>$input</span></div></div>";
		}
		return $input;
	}
	function getCSSClass(){
